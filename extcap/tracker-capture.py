...
if WINDOWS:
    TMP_DIR = os.path.join(os.getenv('APPDATA'), 'Trackershark')
else:
    TMP_DIR = '/tmp/trackershark'
os.makedirs(TMP_DIR, exist_ok=True)

EXTCAP_VERSION = 'VERSION_PLACEHOLDER'
EXTCAP_DISPLAY = 'Tracker'
INTERFACE_NAME = 'tracker'
INTERFACE_DISPLAY = 'Tracker capture'
DLT_USER0 = 147
TRACKER_OUTPUT_BUF_CAPACITY = 262144 # enough to hold the largest event encountered so far
DATA_PORT = 4000
REMOTE_CAPTURE_LOGFILE = '/tmp/tracker_logs.log'
LOCAL_CAPTURE_INSTALL_PATH = os.path.join(TMP_DIR, 'tracker_tmp')
REMOTE_CAPTURE_INSTALL_PATH = '/tmp/tracker_tmp'
REMOTE_CAPTURE_OUTPUT_DIR = '/tmp/tracker_output'
REMOTE_CAPTURE_NEW_ENTRYPOINT = '/tmp/new-entrypoint.sh'
READER_COMM = 'tracker-capture'
PID_FILE = os.path.join(TMP_DIR, "trackershark.pid")

GENERAL_GROUP = 'General'
REMOTE_GROUP = 'Remote capture'
TRACKER_OPTIONS_GROUP = 'Tracker options'

DEFAULT_CAPTURE_TYPE = 'local'
DEFAULT_TRACKER_IMAGE = 'khulnasoft/tracker:0.22.0'
...
DEFAULT_CONTAINER_NAME = 'trackershark'
DEFAULT_LOGFILE = os.path.join(TMP_DIR, 'tracker_logs.log')
DEFAULT_OUTPUT_DIR = os.path.join(TMP_DIR, 'tracker_output')
...
def show_version():
    print("extcap {version=%s}{help=https://www.wireshark.org}{display=Tracker}" % EXTCAP_VERSION)
...
def load_preset(preset_name: str) -> str:
    preset_file = os.path.join(os.path.dirname(__file__), 'tracker-capture', 'presets', preset_name)
    return load_preset_file(preset_file)


def get_effective_tracker_options(args: argparse.Namespace) -> str:
    options = ''
    ...
    # add custom options
    if args.custom_tracker_options:
        options += f' {args.custom_tracker_options}'
...
def get_presets() -> List[str]:
    presets_dir = os.path.join(os.path.dirname(__file__), 'tracker-capture', 'presets')
...
def show_config(reload_option: Optional[str]):
    ...
    args: List[ConfigArg] = [
        ...
        ConfigArg(call='--logfile', display='Tracker logs file', type='fileselect',
            default=DEFAULT_LOGFILE,
            group=GENERAL_GROUP
        ),
        ConfigArg(call='--output-dir', display='Tracker output folder', type='string',
            default=DEFAULT_OUTPUT_DIR,
            group=GENERAL_GROUP
        ),
        ConfigArg(call='--container-image', display='Docker image', type='string',
            tooltip='Tracker docker image',
            default=DEFAULT_TRACKER_IMAGE,
            group=GENERAL_GROUP
        ),
        ...
        ConfigArg(call='--preset', display='Preset', type='selector',
            tooltip='Tracker options preset',
            group=TRACKER_OPTIONS_GROUP,
            reload='true',
        ...
        ConfigArg(call='--custom-tracker-options', display='Custom Tracker options', type='string',
            tooltip='Command line options for Tracker',
            group=TRACKER_OPTIONS_GROUP
        ),
        ...
        ConfigArg(call='--network-snaplen', display='Packet snaplen', type='string',
            ...
            tooltip='Length of captured packets. See the "Forensics" section of Tracker\'s documentation for details',
            group=TRACKER_OPTIONS_GROUP
        ),
    ]
...
def show_dlts():
    print("dlt {number=%d}{name=USER0}{display=Tracker event}" % DLT_USER0)
...
class DataOutputManager:
    ...
    def write_event(self, event: bytes):
        ...
        # Protection against negative timestamp bug in older Tracker version,
...
def read_output(data_output_manager: DataOutputManager):
    ...
    # open tracker output socket and listen for an incoming connection
    tracker_output_sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    tracker_output_sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    tracker_output_sock.bind(('127.0.0.1', DATA_PORT))
    tracker_output_sock.listen(0)
    tracker_output_sock.setsockopt(socket.SOL_SOCKET, socket.SO_RCVBUF, TRACKER_OUTPUT_BUF_CAPACITY)
    tracker_output_sock.settimeout(0.1)

    tracker_output_conn = None
    while running:
        try:
            tracker_output_conn, _ = tracker_output_sock.accept()
            tracker_output_conn.settimeout(0.1)
            break
    ...
    while running:
        try:
            ...
            data = tracker_output_conn.recv(TRACKER_OUTPUT_BUF_CAPACITY)
            unpacker.feed(data)
        except socket.timeout:
            continue
    ...
    if tracker_output_conn is not None:
        tracker_output_conn.close()
    tracker_output_sock.close()
...
def sample_tracker_init(sftp: Optional[SFTPManager]):
    global local

    stat = os.stat if local else sftp.stat
    tracker_pid_file = os.path.join(LOCAL_CAPTURE_INSTALL_PATH, 'tracker.pid') if local else f'{REMOTE_CAPTURE_INSTALL_PATH}/tracker.pid'

    while True:
        try:
            stat(tracker_pid_file)
        except FileNotFoundError:
            pass
        else:
            log("Tracker is up and running")
            return
        
        time.sleep(0.5)


def tracker_init_sampler_thread(sftp: Optional[SFTPManager]):
    try:
        sample_tracker_init(sftp)
    except Exception as ex:
        exception(ex)
...
def build_docker_run_command(args: argparse.Namespace, local: bool, sshd_pid: Optional[int] = None) -> str:
    tracker_options = get_effective_tracker_options(args)

    command = 'docker run -d'

    # when not using docker for Windows or Mac, we connect Tracker to the local network
    ...
    new_entrypoint = os.path.join(os.path.dirname(__file__), 'tracker-capture', 'new-entrypoint.sh') if local else REMOTE_CAPTURE_NEW_ENTRYPOINT
    command += f' {args.docker_options} -v {logfile}:/logs.log:rw -v {install_path}:/install_path:rw -v {output_dir}:/output:rw -v {new_entrypoint}:/new-entrypoint.sh --entrypoint /new-entrypoint.sh {args.container_image} {tracker_options}'

    # add exclusions that may spam the capture
    if sshd_pid is not None:
        command += f' --scope pid!={sshd_pid}'
    
    if 'comm=' not in tracker_options: # make sure there is no comm filter in place, otherwise it will be overriden
        command += f' --scope comm!=tracker'
...
def handle_connection(transport: paramiko.Transport, dst_addr: str, dst_port: int):
    ...
    while running:
        ...
        if sock in r:
            data = sock.recv(TRACKER_OUTPUT_BUF_CAPACITY)
            if len(data) == 0:
                continue
            channel.send(data)
        if channel in r:
            data = channel.recv(TRACKER_OUTPUT_BUF_CAPACITY)
            if len(data) == 0:
                continue
            sock.send(data)
...
def remove_existing_tracker_container(args: argparse.Namespace, ssh_client: Optional[paramiko.SSHClient]):
    if len(args.container_name) > 0:
...
def prepare_local_capture(args: argparse.Namespace):
    ...
    remove_existing_tracker_container(args, None)

    # create empty file to get logs from Tracker
    if os.path.isdir(args.logfile):
        os.rmdir(args.logfile)
    open(args.logfile, 'w').close()

    # create directory for Tracker temp files
    ...
    # create directory for Tracker output files
...
def prepare_remote_capture(args: argparse.Namespace, ssh_client: paramiko.SSHClient, sftp: SFTPManager) -> int:
    # remove container from previous run
    remove_existing_tracker_container(args, ssh_client)

    # remove preexisting Tracker logs file
    if os.path.isdir(args.logfile):
        shutil.rmtree(args.logfile)
    elif os.path.isfile(args.logfile):
        os.remove(args.logfile)
    
    # remove preexisting Tracker output directory
    if os.path.isdir(args.output_dir):
        shutil.rmtree(args.output_dir)
    elif os.path.isfile(args.output_dir):
        os.remove(args.output_dir)
    ...
    # create empty Tracker logs file to be mounted into the Tracker container
    ...
    if returncode != 0:
        error(f'error creating file for Tracker logs, stderr dump:\n{err}')
    
    # create directory for Tracker temp files
    _, err, returncode = send_ssh_command(ssh_client, f'rm -rf {REMOTE_CAPTURE_INSTALL_PATH} && mkdir {REMOTE_CAPTURE_INSTALL_PATH}')
    if returncode != 0:
        error(f'error creating temp directory for Tracker, stderr dump:\n{err}')
    ...
    if returncode != 0:
        error(f'error creating output directory for Tracker, stderr dump:\n{err}')
    
    # copy new container entrypoint
    sftp.put(os.path.join(os.path.dirname(__file__), 'tracker-capture', 'new-entrypoint.sh'), REMOTE_CAPTURE_NEW_ENTRYPOINT)
...
def stop_existing_tracker_capture():
...
def tracker_capture(args: argparse.Namespace):
    ...
    # check if Tracker container image exists locally
    ...
    if args.container_image not in out:
        ...
        log("Pulling Tracker docker image...")
        _, err, returncode = send_command(local, command, ssh_client)
        if returncode != 0:
    ...
    # from this point the stop_capture function is responsible for stopping Tracker's cotainer
    container_id = out.rstrip('\n')

    # monitor for tracker.pid file to know when Tracker is running
    tracker_init_sampler_th = threading.Thread(target=tracker_init_sampler_thread, args=(sftp,), daemon=True)
    tracker_init_sampler_th.start()

    # wait until Tracker exits (triggered by stop_capture or by an error)
    ...
    # copy Tracker logs file and output directory
    if not local:
    ...
    if len(logs_err) > 0:
        error(f'Tracker exited with error message:\n{logs_err}')


def main():
    global args

    parser = argparse.ArgumentParser(prog=os.path.basename(__file__), description='Capture events and packets using Tracker')
    ...
    parser.add_argument('--container-image', type=str, default=DEFAULT_TRACKER_IMAGE)
    ...
    parser.add_argument('--custom-tracker-options', type=str)
