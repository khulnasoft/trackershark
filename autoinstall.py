...
def get_latest_release_info() -> Dict[Any, Any]:
    url = "https://api.github.com/repos/khulnasoft/trackershark/releases/latest"
    headers = {
...
def prompt_selected_asset(assets: Dict[str, str], wireshark_version: Optional[str]) -> Optional[str]:
    if wireshark_version is None:
        print("Trackershark autoinstall was unable to find your Wireshark installation.")
    ...
    print("Trackershark releases were found for the following Wireshark versions. Please select which one to install:")
...
def download_asset(asset_id: str, file_name: str):
    url = f"https://api.github.com/repos/khulnasoft/trackershark/releases/assets/{asset_id}"
    headers = {
...
def main():
    ...
    if len(assets) == 0:
        print("No releases exist for your system. Please open an issue and request a release to be added, or build Trackershark from source.")
        return
