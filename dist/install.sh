...
cp -r profiles/Tracker $HOME/.config/wireshark/profiles/
echo "[*] Installed profile to $HOME/.config/wireshark/profiles/Tracker"
...
cp tracker-event.so* $PLUGINS_DIR/epan
cp tracker-network-capture.so* $PLUGINS_DIR/epan
mkdir -p $PLUGINS_DIR/wiretap
cp tracker-json.so* $PLUGINS_DIR/wiretap
...
mkdir -p $EXTCAP_DIR
cp extcap/tracker-capture.py $EXTCAP_DIR
if [ "$OS_NAME" == "Linux" ]; then
    chmod +x $EXTCAP_DIR/tracker-capture.py
else
    cp extcap/tracker-capture.sh $EXTCAP_DIR
    chmod +x $EXTCAP_DIR/tracker-capture.sh
fi
cp -r extcap/tracker-capture $EXTCAP_DIR
chmod +x $EXTCAP_DIR/tracker-capture/new-entrypoint.sh
