...
- Tracker packet capture context dissection (new Tracker feature)

- Script that merges Tracker pcaps with events
...
- Live capture fix for older Tracker versions
...
- Source and destination IP columns in Tracker profile
...
- Live capture
  ...
  - Tracker options and presets are not mututally exclusive anymore
  
  - Simplified preset system
  
  - Pin default Tracker docker image to current stable release
...
- Tracker event dissector bugfixes
...
- Trackershark version is specified once for building in the `.env` file
...
- Prevent native Wireshark color rules from overriding Tracker color rules

- Prevent live captured events from being identified as packets recorded by Tracker
...
- Tracker network capture dissector is now a postdissector instead of overriding the NULL/Loopback dissector
...
- Ability to load Tracker JSON output files into Wireshark

- Dissection of Tracker JSON events:
...
- Dissection of packet captures generated by Tracker. This dissector adds Tracker context based on the interface description, a feature which is not yet part of Tracker.

- Tracker profile for Wireshark
  
  - Custom columns tailored for Tracker events
...
- Live capture using an extcap that runs a Tracker docker container and streams its events to Wireshark
  
  - Local live capture on Linux
  
  - Ability to specift command line options for Tracker
  
  - Preset system for saved Tracker command lines
