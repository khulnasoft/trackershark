...
DEFAULT_PCAPS_DIR = '/tmp/tracker/out/pcap'
DEFAULT_OUTPUT_FILE = 'merged.pcapng'


def get_pcap_files(tracker_pcaps_dir: str) -> List[str]:
    pcaps: List[str] = []
    subdirs = os.listdir(tracker_pcaps_dir)

    # Prefer per-process PCAPs
    if 'processes' in subdirs:
        for container in os.listdir(os.path.join(tracker_pcaps_dir, 'processes')):
            for pcap in os.listdir(os.path.join(tracker_pcaps_dir, 'processes', container)):
                pcaps.append(os.path.join(tracker_pcaps_dir, 'processes', container, pcap))
    
    # Next preference is per-command PCAPs
    elif 'commands' in subdirs:
        for container in os.listdir(os.path.join(tracker_pcaps_dir, 'commands')):
            for pcap in os.listdir(os.path.join(tracker_pcaps_dir, 'commands', container)):
                pcaps.append(os.path.join(tracker_pcaps_dir, 'commands', container, pcap))
    
    # Next preference is per-container PCAPs
    elif 'containers' in subdirs:
        for pcap in os.listdir(os.path.join(tracker_pcaps_dir, 'containers')):
            pcaps.append(os.path.join(tracker_pcaps_dir, 'containers', pcap))
    
    # Finally we resort to single PCAP
    elif 'single.pcap' in subdirs:
        pcaps.append(os.listdir(tracker_pcaps_dir, 'single.pcap'))
    else:
...
def main():
    parser = argparse.ArgumentParser(description='Merge Tracker events with PCAPs')

    parser.add_argument('-e', '--events', type=str, metavar='FILE', help='Tracker events JSON output (optional)')
    parser.add_argument('-p', '--pcaps', type=str, default=DEFAULT_PCAPS_DIR, metavar='DIR', help=f'Tracker PCAPs output dir (default is {DEFAULT_PCAPS_DIR})')
