...
static int proto_tracker_network_capture = -1;
...
static gint ett_tracker_network_capture = -1;
...
static void dissect_process_info(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, char *if_descr, jsmntok_t *root_tok)
{
    ...
    process_item = proto_tree_add_item(tree, proto_tracker_network_capture, tvb, 0, 0, ENC_NA);
    ...
    pid_format = prefs_get_uint_value("tracker", "pid_format");
...
static void dissect_container_info(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, char *if_descr, jsmntok_t *root_tok)
{
    ...
    container_item = proto_tree_add_item(tree, proto_tracker_network_capture, tvb, 0, 0, ENC_NA);
    ...
    if (id != NULL) {
        if (prefs_get_uint_value("tracker", "container_identifier") == CONTAINER_IDENTIFIER_ID)
        ...
        if (image) {
            if (prefs_get_uint_value("tracker", "container_image"))
...
static void dissect_kubernetes_info(tvbuff_t *tvb, proto_tree *tree, char *if_descr, jsmntok_t *root_tok)
{
    ...
    k8s_item = proto_tree_add_item(tree, proto_tracker_network_capture, tvb, 0, 0, ENC_NA);
...
static int postdissect_tracker_network_capture(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, void *data _U_)
{
    proto_tree *tracker_network_capture_tree;
    proto_item *tracker_network_capture_item, *tmp_item;
    char *if_descr;
    int num_toks;
    jsmntok_t *root_tok;
    gchar *tmp_str;
    ...
    // make sure this is a packet recorded by Tracker
    ...
    if (strcmp(epan_get_interface_name(pinfo->epan, pinfo->rec->rec_header.packet_header.interface_id, section_number), "tracker") != 0)
#else
    if (strcmp(epan_get_interface_name(pinfo->epan, pinfo->rec->rec_header.packet_header.interface_id), "tracker") != 0)
#endif
        return 0;
    ...
    // create tracker network capture tree
    tracker_network_capture_item = proto_tree_add_item(tree, proto_tracker_network_capture, tvb, 0, -1, ENC_NA);
    tracker_network_capture_tree = proto_item_add_subtree(tracker_network_capture_item, ett_tracker_network_capture);

    // move tracker network capture item below frame item
    tmp_item = get_first_item(tree);
    proto_tree_move_item(tree, tmp_item, tracker_network_capture_item);
    ...
    if ((tmp_str = json_get_string(if_descr, root_tok, "version")) != NULL) {
        ...
        dissect_interface_description(tvb, pinfo, tracker_network_capture_tree, if_descr, root_tok, &version);
    }
    // no version field means this is an old exerimental version of packet context, for now this is dissected for compatibility
    else
        dissect_interface_description_compat(tvb, pinfo, tracker_network_capture_tree, tracker_network_capture_item, if_descr, root_tok);

    return 0;
}

void proto_register_tracker_network_capture(void)
{
    dissector_handle_t tracker_network_capture_handle;

    static gint *ett[] = {
        &ett_tracker_network_capture,
        &ett_process,
        &ett_container,
        &ett_k8s,
    };

    proto_tracker_network_capture = proto_register_protocol("Tracker Network Capture", "TRACKER-NETWORK-CAPTURE", "tracker-network-capture");
    proto_register_subtree_array(ett, array_length(ett));

    tracker_network_capture_handle = register_dissector("tracker-network-capture", postdissect_tracker_network_capture, proto_tracker_network_capture);
    register_postdissector(tracker_network_capture_handle);
}

void proto_reg_handoff_tracker_network_capture(void)
{
    // get hf id for tracker event fields we need
    hf_hostname = proto_registrar_get_id_byname("tracker.hostName");
    hf_executable_path = proto_registrar_get_id_byname("tracker.executable.path");
    hf_process_name = proto_registrar_get_id_byname("tracker.processName");
    hf_process_id = proto_registrar_get_id_byname("tracker.processId");
    hf_thread_id = proto_registrar_get_id_byname("tracker.threadId");
    hf_parent_process_id = proto_registrar_get_id_byname("tracker.parentProcessId");
    hf_host_process_id = proto_registrar_get_id_byname("tracker.hostProcessId");
    hf_host_thread_id = proto_registrar_get_id_byname("tracker.hostThreadId");
    hf_host_parent_process_id = proto_registrar_get_id_byname("tracker.hostParentProcessId");
    hf_pid_col = proto_registrar_get_id_byname("tracker.pid_col");
    hf_ppid_col = proto_registrar_get_id_byname("tracker.ppid_col");
    hf_thread_start_time = proto_registrar_get_id_byname("tracker.threadStartTime");
    hf_user_id = proto_registrar_get_id_byname("tracker.userId");
    hf_cgroup_id = proto_registrar_get_id_byname("tracker.cgroupId");
    hf_mount_namespace = proto_registrar_get_id_byname("tracker.mountNamespace");
    hf_pid_namespace = proto_registrar_get_id_byname("tracker.pidNamespace");
    hf_container_id = proto_registrar_get_id_byname("tracker.container.id");
    hf_container_name = proto_registrar_get_id_byname("tracker.container.name");
    hf_container_image = proto_registrar_get_id_byname("tracker.container.image");
    hf_container_image_digest = proto_registrar_get_id_byname("tracker.container.imageDigest");
    hf_is_container = proto_registrar_get_id_byname("tracker.isContainer");
    hf_container_col = proto_registrar_get_id_byname("tracker.container_col");
    hf_k8s_pod_name = proto_registrar_get_id_byname("tracker.kubernetes.podName");
    hf_k8s_pod_namespace = proto_registrar_get_id_byname("tracker.kubernetes.podNamespace");
    hf_k8s_pod_uid = proto_registrar_get_id_byname("tracker.kubernetes.podUID");
}