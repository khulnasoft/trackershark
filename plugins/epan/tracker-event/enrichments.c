...
static int enrich_sched_process_exec(tvbuff_t *tvb _U_, packet_info *pinfo, proto_tree *tree _U_, void *data)
{
    ...
    struct tracker_dissector_data *dissector_data = (struct tracker_dissector_data *)data;

    pathname = wanted_field_get_str("tracker.args.sched_process_exec.pathname");
    cmdline = wanted_field_get_str("tracker.args.command_line");
...
static int enrich_net_packet_http_request(tvbuff_t *tvb _U_, packet_info *pinfo, proto_tree *tree _U_, void *data _U_)
{
    ...
    method = wanted_field_get_str("tracker.proto_http_request.method");
    protocol = wanted_field_get_str("tracker.proto_http_request.protocol");
    uri_path = wanted_field_get_str("tracker.proto_http_request.uri_path");
...
static int enrich_net_packet_http(tvbuff_t *tvb _U_, packet_info *pinfo, proto_tree *tree _U_, void *data _U_)
{
    ...
    direction = wanted_field_get_str("tracker.proto_http.direction");
    if (direction == NULL)
        return 0;
    ...
    protocol = wanted_field_get_str("tracker.proto_http.protocol");
    ...
    if (request) {
        method = wanted_field_get_str("tracker.proto_http.method");
        uri_path = wanted_field_get_str("tracker.proto_http.uri_path");
        ...
        status = wanted_field_get_str("tracker.proto_http.status");
...
gchar *enrichments_get_security_socket_bind_connect_description(packet_info *pinfo, const gchar *verb)
{
    ...
    if ((family = wanted_field_get_str("tracker.sockaddr.sa_family")) == NULL)
        return NULL;
    
    if (strcmp(family, "AF_INET") == 0) {
        addr = wanted_field_get_str("tracker.sockaddr.sin_addr");
        port = wanted_field_get_str("tracker.sockaddr.sin_port");
    }
    else if (strcmp(family, "AF_INET6") == 0) {
        addr = wanted_field_get_str("tracker.sockaddr.sin6_addr");
        port = wanted_field_get_str("tracker.sockaddr.sin6_port");
    }
    else if (strcmp(family, "AF_UNIX") == 0) {
        addr = wanted_field_get_str("tracker.sockaddr.sun_path");
    }
    else
        return NULL;
...
{
    const gchar *alert = wanted_field_get_str("tracker.args.dynamic_code_loading.triggered_by.alert");
...
{
    const gchar *pathname = wanted_field_get_str("tracker.args.fileless_execution.triggered_by.pathname");
...
static int enrich_stdio_over_socket(tvbuff_t *tvb _U_, packet_info *pinfo, proto_tree *tree _U_, void *data _U_)
{
    ...
    fd = wanted_field_get_int("tracker.args.stdio_over_socket.File_descriptor");
    addr = wanted_field_get_str("tracker.args.stdio_over_socket.IP_address");
    port = wanted_field_get_str("tracker.args.stdio_over_socket.Port");
...
static int enrich_magic_write(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree _U_, void *data)
{
    ...
    if ((bytes = wanted_field_get_str("tracker.args.magic_write.bytes")) == NULL)
        return 0;
    ...
    pathname = wanted_field_get_str("tracker.args.magic_write.pathname");
...
static int enrich_security_file_open(tvbuff_t *tvb _U_, packet_info *pinfo, proto_tree *tree _U_, void *data _U_)
{
    ...
    if ((syscall_pathname = wanted_field_get_str("tracker.args.security_file_open.syscall_pathname")) == NULL)
        return 0;

    col_add_fstr(pinfo->cinfo, COL_INFO, "Open %s", syscall_pathname);

    if ((pathname = wanted_field_get_str("tracker.args.security_file_open.pathname")) == NULL)
        return 0;
...
static void register_wanted_fields(void)
{
    ...
    register_wanted_field("tracker.args.sched_process_exec.pathname");
    register_wanted_field("tracker.args.command_line");

    // needed for enrich_net_packet_http_request
    register_wanted_field("tracker.proto_http_request.method");
    register_wanted_field("tracker.proto_http_request.protocol");
    register_wanted_field("tracker.proto_http_request.uri_path");
    register_wanted_field("http.content_type");

    // needed for enrich_net_packet_http
    register_wanted_field("tracker.proto_http.direction");
    register_wanted_field("tracker.proto_http.method");
    register_wanted_field("tracker.proto_http.protocol");
    register_wanted_field("tracker.proto_http.uri_path");
    register_wanted_field("tracker.proto_http.status");

    // needed for enrich_security_socket_bind_connect
    register_wanted_field("tracker.sockaddr.sa_family");
    register_wanted_field("tracker.sockaddr.sin_addr");
    register_wanted_field("tracker.sockaddr.sin_port");
    register_wanted_field("tracker.sockaddr.sin6_addr");
    register_wanted_field("tracker.sockaddr.sin6_port");
    register_wanted_field("tracker.sockaddr.sun_path");

    // needed for enrich_dynamic_code_loading
    register_wanted_field("tracker.args.dynamic_code_loading.triggered_by.alert");

    // needed for enrich_fileless_execution
    register_wanted_field("tracker.args.fileless_execution.triggered_by.pathname");

    // needed for enrich_stdio_over_socket
    register_wanted_field("tracker.args.stdio_over_socket.File_descriptor");
    register_wanted_field("tracker.args.stdio_over_socket.IP_address");
    register_wanted_field("tracker.args.stdio_over_socket.Port");

    // needed for enrich_magic_write
    register_wanted_field("tracker.args.magic_write.bytes");
    register_wanted_field("tracker.args.magic_write.pathname");

    // needed for enrich_security_file_open
    register_wanted_field("tracker.args.security_file_open.pathname");
    register_wanted_field("tracker.args.security_file_open.syscall_pathname");
}

void register_tracker_enrichments(int proto)
{
    static hf_register_info hf[] = {
        { &hf_decoded_data,
          { "Decoded data", "tracker.args.magic_write.decoded_data",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            NULL, HFILL }
        },
        { &hf_file_type,
          { "File type", "tracker.args.magic_write.file_type",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            NULL, HFILL }
        },
    };
...
static void register_tracker_event_enrichment(const gchar *event_name, dissector_t dissector)
{
    int proto_tracker;
    dissector_handle_t dissector_handle;
    
    DISSECTOR_ASSERT((proto_tracker = proto_get_id_by_filter_name("tracker")) != -1);

    dissector_handle = create_dissector_handle(dissector, proto_tracker);
    dissector_add_string("tracker.eventName", event_name, dissector_handle);
}

void proto_reg_handoff_tracker_enrichments(void)
{
    register_tracker_event_enrichment("sched_process_exec", enrich_sched_process_exec);
    register_tracker_event_enrichment("net_packet_http_request", enrich_net_packet_http_request);
    register_tracker_event_enrichment("net_packet_http", enrich_net_packet_http);
    register_tracker_event_enrichment("security_socket_bind", enrich_security_socket_bind);
    register_tracker_event_enrichment("security_socket_connect", enrich_security_socket_connect);
    register_tracker_event_enrichment("dynamic_code_loading", enrich_dynamic_code_loading);
    register_tracker_event_enrichment("fileless_execution", enrich_fileless_execution);
    register_tracker_event_enrichment("stdio_over_socket", enrich_stdio_over_socket);
    register_tracker_event_enrichment("magic_write", enrich_magic_write);
    register_tracker_event_enrichment("security_file_open", enrich_security_file_open);
}