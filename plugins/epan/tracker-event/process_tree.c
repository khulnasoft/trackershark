...
void process_tree_init(void)
{
    ...
    register_wanted_field("tracker.args.sched_process_fork.child_pid");
    register_wanted_field("tracker.args.sched_process_fork.child_tid");
}

void process_tree_update(struct tracker_dissector_data *data)
{
    ...
    if (strcmp(data->event_name, "sched_process_fork") == 0) {
        DISSECTOR_ASSERT((fork_child_pid = wanted_field_get_int("tracker.args.sched_process_fork.child_pid")) != NULL);
        DISSECTOR_ASSERT((fork_child_tid = wanted_field_get_int("tracker.args.sched_process_fork.child_tid")) != NULL);
