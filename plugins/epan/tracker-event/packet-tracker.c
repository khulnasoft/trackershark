...
static int proto_tracker = -1;
static int tracker_tap;
...
/**
 * This map serves as a set of all of Tracker's string types.
 * Most of these are syscall fields with a string representing a memory address.
 */
static wmem_map_t *string_types;
...
static void dissect_arguments(tvbuff_t *, packet_info *, proto_tree *, gchar *,
    jsmntok_t *, const gchar *, gboolean, struct tracker_dissector_data *);
...
static void free_dynamic_hf(gpointer key _U_, gpointer value, gpointer user_data _U_)
{
    ...
    for (i = 0; i < dynamic_hf->hf_ptrs->len; i++) {
        ...
        proto_deregister_field(proto_tracker, *(hf->p_id));
    }
...
static void dissect_process_fields(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree,
    gchar *json_data, jsmntok_t *root_tok, struct tracker_dissector_data *data)
{
    ...
    process_item = proto_tree_add_item(tree, proto_tracker, tvb, 0, 0, ENC_NA);
...
static void dissect_container_fields(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, gchar *json_data, jsmntok_t *root_tok)
{
    ...
    container_item = proto_tree_add_item(tree, proto_tracker, tvb, 0, 0, ENC_NA);
...
static void dissect_k8s_fields(tvbuff_t *tvb, proto_tree *tree, gchar *json_data, jsmntok_t *root_tok)
{
    ...
    k8s_item = proto_tree_add_item(tree, proto_tracker, tvb, 0, 0, ENC_NA);
    ...
    gchar *json_data, jsmntok_t *root_tok, struct tracker_dissector_data *data)
{
    ...
    context_item = proto_tree_add_item(tree, proto_tracker, tvb, 0, 0, ENC_NA);
...
static void get_arg_field_type_display(const gchar *type, struct type_display *info, const gchar *event_name, const gchar *arg_name)
{
    ...
    // string
    if (wmem_map_contains(string_types, type)) {
...
static void dynamic_hf_populate_arg_field(hf_register_info *hf, const gchar *event_name, const gchar *arg_name, const gchar *type)
{
    ...
    *(hf->p_id) = -1;
    ...
    hf->hfinfo.abbrev = g_strdup_printf("tracker.args.%s.%s", event_name, name_normalized);
...
static hf_register_info *get_arg_hf(const gchar *event_name, gchar *json_data, jsmntok_t *arg_tok)
{
    ...
    *hf_idx = dynamic_hf->hf_ptrs->len - 1;
    ...
    proto_register_field_array(proto_tracker, hf, 1);
    ...
    const gchar *arr_name, gchar *json_data, jsmntok_t *arr_tok, wmem_array_t *arr_data)
{
    ...
    for (i = 0; i < arr_len; i++) {
        ...
        DISSECTOR_ASSERT(elem_tok->type == JSMN_PRIMITIVE);
    ...
    const gchar *arr_name, gchar *json_data, jsmntok_t *parent_tok, const gchar *arr_tok_name, gboolean get_data)
{
    ...
    // create the item
    arr_item = proto_tree_add_item(tree, proto_tracker, tvb, 0, 0, ENC_NA);
...
static gchar *do_dissect_sockaddr(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, gchar *json_data, jsmntok_t *obj_tok)
{
    ...
    arg_str = wmem_strdup_printf(pinfo->pool, "sa_family = %s", tmp_str);
    ...
    if ((tmp_str = json_get_string(json_data, obj_tok, "sun_path")) != NULL) {
        ...
        arg_str = wmem_strdup_printf(pinfo->pool, "%s, sun_path = %s", arg_str, tmp_str);
    }
    ...
    if ((tmp_str = json_get_string(json_data, obj_tok, "sin_addr")) != NULL) {
        ...
        arg_str = wmem_strdup_printf(pinfo->pool, "%s, sin_addr = %s", arg_str, tmp_str);
    ...
    if ((tmp_str = json_get_string(json_data, obj_tok, "sin_port")) != NULL) {
        ...
        arg_str = wmem_strdup_printf(pinfo->pool, "%s, sin_port = %s", arg_str, tmp_str);
        errno = 0;
    ...
    if ((tmp_str = json_get_string(json_data, obj_tok, "sin6_addr")) != NULL) {
        ...
        arg_str = wmem_strdup_printf(pinfo->pool, "%s, sin6_addr = %s", arg_str, tmp_str);
    ...
    if ((tmp_str = json_get_string(json_data, obj_tok, "sin6_port")) != NULL) {
        ...
        arg_str = wmem_strdup_printf(pinfo->pool, "%s, sin6_port = %s", arg_str, tmp_str);
        errno = 0;
        port = (guint32)strtoll(tmp_str, NULL, 10);
        DISSECTOR_ASSERT(errno == 0);
    }
    ...
    if ((tmp_str = json_get_string(json_data, obj_tok, "sin6_flowinfo")) != NULL) {
        ...
        arg_str = wmem_strdup_printf(pinfo->pool, "%s, sin6_flowinfo = %s", arg_str, tmp_str);
    }
    
    // add sin6_scopeid
    if ((tmp_str = json_get_string(json_data, obj_tok, "sin6_scopeid")) != NULL) {
        arg_str = wmem_strdup_printf(pinfo->pool, "%s, sin6_scopeid = %s", arg_str, tmp_str);
...
static gchar *do_dissect_pktmeta(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, gchar *json_data, jsmntok_t *obj_tok)
{
    ...
    arg_str = wmem_strdup_printf(pinfo->pool, "src_ip = %s", tmp_str);
    ...
    arg_str = wmem_strdup_printf(pinfo->pool, "%s, dst_ip = %s", arg_str, tmp_str);
    if (ws_inet_pton4(tmp_str, &in4_addr)) {
        tmp_item = proto_tree_add_ipv4(tree, hf_ip_addr, tvb, 0, 0, in4_addr);
        proto_item_set_hidden(tmp_item);
        tmp_item = proto_tree_add_ipv4(tree, hf_ip_dst, tvb, 0, 0, in4_addr);
    ...
    arg_str = wmem_strdup_printf(pinfo->pool, "%s, src_port = %u", arg_str, src_port);
    ...
    arg_str = wmem_strdup_printf(pinfo->pool, "%s, dst_port = %u", arg_str, dst_port);
    ...
    arg_str = wmem_strdup_printf(pinfo->pool, "%s, ipproto = %u", arg_str, (guint32)tmp_int);
    ...
    arg_str = wmem_strdup_printf(pinfo->pool, "%s, len = %u", arg_str, (guint32)tmp_int);
    ...
    arg_str = wmem_strdup_printf(pinfo->pool, "%s, iface = %s", arg_str, tmp_str);

    return arg_str;
}

static gchar *do_dissect_dns_query_data(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, gchar *json_data, jsmntok_t *obj_tok)
{
    gchar *arg_str, *tmp_str;
    proto_item *tmp_item;
    ...
    proto_item_set_hidden(tmp_item);
    arg_str = wmem_strdup_printf(pinfo->pool, "query = %s", tmp_str);
    ...
    arg_str = wmem_strdup_printf(pinfo->pool, "%s, type = %s", arg_str, tmp_str);
    ...
    arg_str = wmem_strdup_printf(pinfo->pool, "%s, class = %s", arg_str, tmp_str);
...
static gchar *do_dissect_dns_response_data(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, gchar *json_data, jsmntok_t *obj_tok)
{
    ...
    query_item = proto_tree_add_item(tree, proto_tracker, tvb, 0, 0, ENC_NA);
    ...
    answers_item = proto_tree_add_item(tree, proto_tracker, tvb, 0, 0, ENC_NA);
    ...
    for (i = 0; i < answers_len; i++) {
        // add item subtree
        answer_item = proto_tree_add_item(answers_tree, proto_tracker, tvb, 0, 0, ENC_NA);
...
static gchar *dissect_http_headers(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, gchar *json_data, jsmntok_t *headers_tok)
{
    ...
    // add headers tree
    headers_item = proto_tree_add_item(tree, proto_tracker, tvb, 0, 0, ENC_NA);
    ...
    for (i = 0; i < headers_tok->size; i++, curr_header_tok = json_get_next_object(curr_header_tok)) {
        ...
        for (j = 0; j < curr_arr_size; j++) {
            ...
            tmp_str = wmem_strdup_printf(pinfo->pool, "%s: %s", header_name, header_value);
            ...
            if (headers_str == NULL)
                headers_str = wmem_strdup(pinfo->pool, tmp_str);
            else
                headers_str = wmem_strdup_printf(pinfo->pool, "%s, %s", headers_str, tmp_str);
            ...
            tmp_str = wmem_strdup_printf(pinfo->pool, "%s\r\n", tmp_str);
...
static gchar *do_dissect_proto_http_request(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, gchar *json_data, jsmntok_t *obj_tok)
{
    ...
    proto_item_set_hidden(tmp_item);
    arg_str = wmem_strdup_printf(pinfo->pool, "method = %s", tmp_str);
    ...
    proto_item_set_hidden(tmp_item);
    arg_str = wmem_strdup_printf(pinfo->pool, "%s, protocol = %s", arg_str, tmp_str);
    ...
    proto_item_set_hidden(tmp_item);
    arg_str = wmem_strdup_printf(pinfo->pool, "%s, host = %s", arg_str, tmp_str);
    ...
    proto_item_set_hidden(tmp_item);
    arg_str = wmem_strdup_printf(pinfo->pool, "%s, URI = %s", arg_str, tmp_str);
    ...
    if (headers_str != NULL)
        arg_str = wmem_strdup_printf(pinfo->pool, "%s, headers = [%s]", arg_str, headers_str);
    ...
    proto_item_set_hidden(tmp_item);
    arg_str = wmem_strdup_printf(pinfo->pool, "%s, content_length = %" PRId64, arg_str, tmp_int);

    return arg_str;
}

static gchar *do_dissect_packet_metadata(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, gchar *json_data, jsmntok_t *obj_tok)
{
    gint64 tmp_int;
    const gchar *tmp_str;
    ...
    if (tmp_str != NULL)
        return wmem_strdup_printf(pinfo->pool, "direction = %s", tmp_str);
    else
        return wmem_strdup_printf(pinfo->pool, "direction = %d", (gint32)tmp_int);
}

static gchar *do_dissect_proto_http(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, gchar *json_data, jsmntok_t *obj_tok)
{
    ...
    arg_str = wmem_strdup_printf(pinfo->pool, "direction = %s", tmp_str);
    ...
    if (strlen(tmp_str) > 0)
        arg_str = wmem_strdup_printf(pinfo->pool, "%s, method = %s", arg_str, tmp_str);
    ...
    if (strlen(tmp_str) > 0)
        arg_str = wmem_strdup_printf(pinfo->pool, "%s, status = %s", arg_str, tmp_str);
...
static gchar *do_dissect_hooked_symbol_data(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, gchar *json_data, jsmntok_t *obj_tok)
{
    ...
    arg_str = wmem_strdup_printf(pinfo->pool, "SymbolName = %s", tmp_str);
    ...
    arg_str = wmem_strdup_printf(pinfo->pool, "%s, ModuleOwner = %s", arg_str, tmp_str);
    ...
    gchar *json_data, jsmntok_t *arg_tok, const gchar *arg_name, object_dissector_t dissector)
{
    ...
    obj_item = proto_tree_add_item(tree, proto_tracker, tvb, 0, 0, ENC_NA);
    ...
    if (arg_str != NULL)
        arg_str = wmem_strdup_printf(pinfo->pool, "{%s}", arg_str);
    ...
    gchar *json_data, jsmntok_t *arg_tok, const gchar *arg_name, object_dissector_t dissector)
{
    ...
    for (i = 0; i < arr_len; i++) {
        // add item subtree
        obj_item = proto_tree_add_item(arr_tree, proto_tracker, tvb, 0, 0, ENC_NA);
        ...
        if (arg_str != NULL) {
            ...
            if (arg_arr_str == NULL)
                arg_arr_str = wmem_strdup_printf(pinfo->pool, "[{%s}", arg_str);
            else
                arg_arr_str = wmem_strdup_printf(pinfo->pool, "%s, {%s}", arg_arr_str, arg_str);
        }
    }
    
    // finalize array str
    if (arg_arr_str != NULL)
        arg_arr_str = wmem_strdup_printf(pinfo->pool, "%s]", arg_arr_str);
    ...
    proto_tree *tree, gchar *json_data, jsmntok_t *arg_tok)
{
    ...
    process_lineage_item = proto_tree_add_item(tree, proto_tracker, tvb, 0, 0, ENC_NA);
    ...
    for (i = 0; i < arr_len; i++) {
        ...
        process_item = proto_tree_add_item(process_lineage_tree, proto_tracker, tvb, 0, 0, ENC_NA);
        proto_item_set_text(process_item, "Process");
...
static gchar *dissect_string_array(tvbuff_t *tvb, packet_info *pinfo,
    proto_tree *tree, hf_register_info *hf, gchar *json_data, jsmntok_t *arg_tok)
{
    wmem_array_t *arr;
    unsigned int i;
    ...
    proto_tree *tree, hf_register_info *hf, gchar *json_data, jsmntok_t *arg_tok)
{
    ...
    symbols_item = proto_tree_add_item(tree, proto_tracker, tvb, 0, 0, ENC_NA);
    ...
    for (i = 0; i < obj_tok->size; i++, curr_entry_tok = json_get_next_object(curr_entry_tok)) {
        ...
        if (symbol_data_str != NULL) {
            if (arg_str != NULL)
                arg_str = wmem_strdup_printf(pinfo->pool, "%s, %s: {%s}", arg_str, entry_name, symbol_data_str);
            else
                arg_str = wmem_strdup_printf(pinfo->pool, "{%s: {%s}", entry_name, symbol_data_str);
        }
    }
...
static void dissect_triggered_by(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree,
    gchar *json_data, jsmntok_t *arg_tok, const gchar *event_name, struct tracker_dissector_data *data)
{
    ...
    triggered_by_item = proto_tree_add_item(tree, proto_tracker, tvb, 0, 0, ENC_NA);
...
static void dissect_arguments(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, gchar *json_data,
    jsmntok_t *root_tok, const gchar *event_name, gboolean set_info, struct tracker_dissector_data *data)
{
    ...
    args_item = proto_tree_add_item(tree, proto_tracker, tvb, 0, 0, ENC_NA);
    ...
    for (i = 0; i < nargs; i++) {
        ...
        // parse value according to type
            ...
            switch (hf->hfinfo.type) {
                ...
                // double
                case FT_DOUBLE:
                    DISSECTOR_ASSERT(json_get_double(json_data, curr_arg, "value", &tmp_double));
                    proto_tree_add_double_wanted(args_tree, *(hf->p_id), tvb, 0, 0, tmp_double);
                    arg_str = wmem_strdup_printf(pinfo->pool, "%f", tmp_double);
                    break;
                ...
                case FT_NONE:
                    ws_info("cannot dissect arg \"%s\" of unsupported type \"%s\" in event \"%s\"",
                        hf->hfinfo.name, arg_type, event_name);
    ...
    gchar *json_data, jsmntok_t *root_tok, struct tracker_dissector_data *data)
{
    ...
    col_set_str(pinfo->cinfo, COL_PROTOCOL, "TRACKER/SIG");

    metadata_item = proto_tree_add_item(tree, proto_tracker, tvb, 0, 0, ENC_NA);
...
static void dissect_event_fields(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree,
    proto_item *item, gchar *json_data, struct tracker_dissector_data *data)
{
    int num_toks;
    jsmntok_t *root_tok;
...
static int dissect_tracker_json(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, void *data _U_)
{
    proto_item *tracker_json_item;
    proto_tree *tracker_json_tree;
    guint len;
    gchar *json_data;
    struct tracker_dissector_data *dissector_data;

    col_set_str(pinfo->cinfo, COL_PROTOCOL, "TRACKER");

    // create tracker tree
    tracker_json_item = proto_tree_add_item(tree, proto_tracker, tvb, 0, -1, ENC_NA);
    tracker_json_tree = proto_item_add_subtree(tracker_json_item, ett_tracker);

    proto_item_set_text(tracker_json_tree, "Tracker Event (JSON)");
    ...
    dissector_data = wmem_new0(pinfo->pool, struct tracker_dissector_data);
    dissector_data->process = wmem_new0(pinfo->pool, struct process_info);
    dissect_event_fields(tvb, pinfo, tracker_json_tree, tracker_json_item, json_data, dissector_data);
    ...
    tap_queue_packet(tracker_tap, pinfo, dissector_data);

    return tvb_captured_length(tvb);
}

static void add_string_type(const gchar *type) {
    wmem_map_insert(string_types, type, NULL);
}

static void init_string_types(void) {
    string_types = wmem_map_new(wmem_epan_scope(), g_str_hash, g_str_equal);

    add_string_type("const char *");
    add_string_type("const char*");
    add_string_type("string");
    add_string_type("char*");
    add_string_type("bytes");
    add_string_type("void*");
    add_string_type("int*");
    add_string_type("struct epoll_event*");
    add_string_type("const sigset_t*");
    add_string_type("const struct timespec*");
    add_string_type("struct statx*");
    add_string_type("struct pollfd*");
    add_string_type("struct itimerval*");
    add_string_type("struct msghdr*");
    add_string_type("const struct iovec*");
    add_string_type("const struct itimerspec*");
    add_string_type("struct itimerspec*");
    add_string_type("struct stat*");
    add_string_type("fd_set*");
    add_string_type("sigset_t*");
    add_string_type("const void*");
    add_string_type("struct mmsghdr*");
    add_string_type("struct timex*");
    add_string_type("struct robust_list_head*");
    add_string_type("const struct sigaction*");
    add_string_type("struct sigaction*");
    add_string_type("struct rseq*");
    add_string_type("const struct rlimit64*");
    add_string_type("struct rlimit64*");
    add_string_type("struct linux_dirent64*");
    add_string_type("const stack_t*");
    add_string_type("stack_t*");
    add_string_type("struct statfs*");
    add_string_type("struct rusage*");
    add_string_type("struct sysinfo*");
...
void proto_register_tracker(void)
{
    static gint *ett[] = {
        &ett_tracker,
        &ett_context,
    ...
    static hf_register_info hf[] = {
        ...
        { &hf_thread_start_time,
          { "Thread Start Time", "tracker.threadStartTime",
            FT_ABSOLUTE_TIME, ABSOLUTE_TIME_UTC, NULL, 0,
            NULL, HFILL }
        },
        { &hf_processor_id,
          { "Processor ID", "tracker.processorId",
            FT_INT64, BASE_DEC, NULL, 0,
            "Processor (CPU) ID", HFILL }
        },
        { &hf_cgroup_id,
          { "Cgroup ID", "tracker.cgroupId",
            FT_INT64, BASE_DEC, NULL, 0,
            NULL, HFILL }
        },
        { &hf_process_id,
          { "Process ID", "tracker.processId",
            FT_INT32, BASE_DEC, NULL, 0,
            "Process ID (in PID namespace)", HFILL }
        },
        { &hf_thread_id,
          { "Thread ID", "tracker.threadId",
            FT_INT32, BASE_DEC, NULL, 0,
            "Thread ID (in PID namespace)", HFILL }
        },
        { &hf_parent_process_id,
          { "Parent Process ID", "tracker.parentProcessId",
            FT_INT32, BASE_DEC, NULL, 0,
            "Parent process ID (in PID namespace)", HFILL }
        },
        { &hf_host_process_id,
          { "Host Process ID", "tracker.hostProcessId",
            FT_INT32, BASE_DEC, NULL, 0,
            "Process ID (in root PID namespace, a.k.a host)", HFILL }
        },
        { &hf_pid_col,
          { "PID Column", "tracker.pid_col",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            NULL, HFILL }
        },
        { &hf_ppid_col,
          { "PPID Column", "tracker.ppid_col",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            NULL, HFILL }
        },
        { &hf_host_thread_id,
          { "Host Thread ID", "tracker.hostThreadId",
            FT_INT32, BASE_DEC, NULL, 0,
            "Thread ID (in root PID namespace, a.k.a host)", HFILL }
        },
        { &hf_host_parent_process_id,
          { "Host Parent Process ID", "tracker.hostParentProcessId",
            FT_INT32, BASE_DEC, NULL, 0,
        ...
        { &hf_mount_namespace,
          { "Mount Namespace", "tracker.mountNamespace",
            FT_UINT32, BASE_DEC, NULL, 0,
            NULL, HFILL }
        },
        { &hf_pid_namespace,
          { "PID Namespace", "tracker.pidNamespace",
            FT_UINT32, BASE_DEC, NULL, 0,
            NULL, HFILL }
        },
        { &hf_process_name,
          { "Process Name", "tracker.processName",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            NULL, HFILL }
        },
        { &hf_executable_path,
          { "Executable Path", "tracker.executable.path",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            NULL, HFILL }
        },
        { &hf_hostname,
        ...
        { &hf_container_name,
          { "Container Name", "tracker.container.name",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            NULL, HFILL }
        },
        { &hf_container_image,
          { "Container Image", "tracker.container.image",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            NULL, HFILL }
        },
        { &hf_container_image_digest,
          { "Container Image Digest", "tracker.container.imageDigest",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            NULL, HFILL }
        },
        { &hf_is_container,
          { "Is Container", "tracker.isContainer",
            FT_BOOLEAN, BASE_NONE, NULL, 0,
            "This event happened inside a container", HFILL }
        },
        { &hf_container_col,
          { "Container Column", "tracker.container_col",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            NULL, HFILL }
        },
        { &hf_k8s_pod_name,
          { "Pod Name", "tracker.kubernetes.podName",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            NULL, HFILL }
        },
        { &hf_k8s_pod_namespace,
          { "Pod Namespace", "tracker.kubernetes.podNamespace",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            NULL, HFILL }
        },
        { &hf_k8s_pod_uid,
          { "Pod UID", "tracker.kubernetes.podUID",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            NULL, HFILL }
        },
        { &hf_event_id,
          { "Event ID", "tracker.eventId",
            FT_INT64, BASE_DEC, NULL, 0,
            "Tracker event ID", HFILL }
        },
        { &hf_event_name,
          { "Event Name", "tracker.eventName",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "Tracker event name", HFILL }
        },
        { &hf_matched_policies,
          { "Matched Policies", "tracker.matchedPolicies",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "Matched Tracker policies", HFILL }
        },
        { &hf_args_num,
          { "Args Num", "tracker.argsNum",
            FT_INT64, BASE_DEC, NULL, 0,
            "Number of arguments", HFILL }
        },
        { &hf_is_signature,
          { "Is Signature", "tracker.isSignature",
            FT_BOOLEAN, BASE_NONE, NULL, 0,
            "This event is a signature", HFILL }
        },
        { &hf_return_value,
          { "Return Value", "tracker.returnValue",
            FT_INT32, BASE_DEC, NULL, 0,
            "Return value of the hooked function or syscall", HFILL }
        },
        { &hf_syscall,
          { "Syscall", "tracker.syscall",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "Originating syscall", HFILL }
        },
        { &hf_thread_entity_id,
          { "Thread Entity ID", "tracker.threadEntityId",
            FT_INT64, BASE_DEC, NULL, 0,
            NULL, HFILL }
        },
        { &hf_process_entity_id,
          { "Process Entity ID", "tracker.processEntityId",
            FT_INT64, BASE_DEC, NULL, 0,
            NULL, HFILL }
        },
        { &hf_parent_entity_id,
          { "Parent Entity ID", "tracker.parentEntityId",
            FT_INT64, BASE_DEC, NULL, 0,
            NULL, HFILL }
        },
        { &hf_args_command_line,
          { "Command Line", "tracker.args.command_line",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "Process arguments", HFILL }
        },
        { &hf_process_lineage_pid,
          { "PID", "tracker.process_lineage.pid",
            FT_INT32, BASE_DEC, NULL, 0,
            "Process ID (in root PID namespace, a.k.a host)", HFILL }
        },
        { &hf_process_lineage_ppid,
          { "PPID", "tracker.process_lineage.ppid",
            FT_INT32, BASE_DEC, NULL, 0,
            "Parent process ID (in root PID namespace, a.k.a host)", HFILL }
        },
        { &hf_process_lineage_start_time,
          { "Start Time", "tracker.process_lineage.start_time",
            FT_ABSOLUTE_TIME, ABSOLUTE_TIME_UTC, NULL, 0,
            NULL, HFILL }
        },
        { &hf_process_lineage_process_name,
          { "Process Name", "tracker.process_lineage.process_name",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            NULL, HFILL }
        },
        { &hf_process_lineage_pathname,
          { "Pathname", "tracker.process_lineage.pathname",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "Process executable path", HFILL }
        },
        { &hf_process_lineage_sha256,
          { "SHA256", "tracker.process_lineage.sha256",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "SHA256 hash of the process executable", HFILL }
        },
        { &hf_process_lineage_command,
          { "Command", "tracker.process_lineage.command",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "Process command line", HFILL }
        },
        { &hf_tiggered_by_id,
          { "Event ID", "tracker.triggered_by.id",
            FT_INT64, BASE_DEC, NULL, 0,
            "ID of the event that triggered the signature", HFILL }
        },
        { &hf_tiggered_by_name,
          { "Event Name", "tracker.triggered_by.name",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "Name of the event that triggered the signature", HFILL }
        },
        { &hf_tiggered_by_return_value,
          { "Return Value", "tracker.triggered_by.return_value",
            FT_INT64, BASE_DEC, NULL, 0,
        ...
        { &hf_metadata_description,
          { "Description", "tracker.metadata.Description",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "Signature description", HFILL }
        },
        { &hf_metadata_properties_category,
          { "Category", "tracker.metadata.Properties.Category",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "Signature category", HFILL }
        },
        { &hf_metadata_properties_kubernetes_technique,
          { "Kubernetes Technique", "tracker.metadata.Properties.Kubernetes_Technique",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            NULL, HFILL }
        },
        { &hf_metadata_properties_severity,
          { "Severity", "tracker.metadata.Properties.Severity",
            FT_INT32, BASE_DEC, NULL, 0,
            "Severity of the signature (0 is lowest, 4 is highest)", HFILL }
        },
        { &hf_metadata_properties_technique,
          { "Technique", "tracker.metadata.Properties.Technique",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            NULL, HFILL }
        },
        { &hf_metadata_properties_aggregation_keys,
          { "Aggregation Keys", "tracker.metadata.Properties.aggregation_keys",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            NULL, HFILL }
        },
        { &hf_metadata_properties_external_id,
          { "External ID", "tracker.metadata.Properties.external_id",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "External technique ID (e.g. MITRE)", HFILL }
        },
        { &hf_metadata_properties_id,
          { "Unified External ID", "tracker.metadata.Properties.id",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            NULL, HFILL }
        },
        { &hf_metadata_properties_release,
          { "Release", "tracker.metadata.Properties.release",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            NULL, HFILL }
        },
        { &hf_metadata_properties_signature_id,
          { "Internal ID", "tracker.metadata.Properties.signatureID",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            NULL, HFILL }
        },
        { &hf_metadata_properties_signature_name,
          { "Signature Name", "tracker.metadata.Properties.signatureName",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            NULL, HFILL }
        }
    };
    ...
    static hf_register_info filter_hf[] = {
        { &hf_ptrace_request,
          { "request", "tracker.args.ptrace.request",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            NULL, HFILL }
        },
    };
    ...
    static hf_register_info sockaddr_hf[] = {
        { &hf_sockaddr_sa_family,
          { "sa_family", "tracker.sockaddr.sa_family",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "Socket family", HFILL }
        },
        { &hf_sockaddr_sun_path,
          { "sun_path", "tracker.sockaddr.sun_path",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "Unix socket path", HFILL }
        },
        { &hf_sockaddr_sin_addr,
          { "sin_addr", "tracker.sockaddr.sin_addr",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "Socket IPv4 address", HFILL }
        },
        { &hf_sockaddr_sin_port,
          { "sin_port", "tracker.sockaddr.sin_port",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "Socket TCP/UDP port", HFILL }
        },
        { &hf_sockaddr_sin6_addr,
          { "sin6_addr", "tracker.sockaddr.sin6_addr",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "Socket IPv6 address", HFILL }
        },
        { &hf_sockaddr_sin6_port,
          { "sin6_port", "tracker.sockaddr.sin6_port",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "Socket TCP/UDP port", HFILL }
        },
        { &hf_sockaddr_sin6_flowinfo,
          { "sin6_flowinfo", "tracker.sockaddr.sin6_flowinfo",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "Socket IPv6 flow info", HFILL }
        },
        { &hf_sockaddr_sin6_scopeid,
          { "sin6_scopeid", "tracker.sockaddr.sin6_scopeid",
            FT_STRINGZ, BASE_NONE, NULL, 0,
    ...
    static hf_register_info slim_cred_t_hf[] = {
        { &hf_slim_cred_t_uid,
          { "UID", "tracker.slim_cred_t.uid",
            FT_INT64, BASE_DEC, NULL, 0,
            "Real user ID", HFILL }
        },
        { &hf_slim_cred_t_gid,
          { "GID", "tracker.slim_cred_t.gid",
            FT_INT64, BASE_DEC, NULL, 0,
            "Real group ID", HFILL }
        },
        { &hf_slim_cred_t_suid,
          { "SUID", "tracker.slim_cred_t.suid",
            FT_INT64, BASE_DEC, NULL, 0,
            "Saved user ID", HFILL }
        },
        { &hf_slim_cred_t_sgid,
          { "SGID", "tracker.slim_cred_t.sgid",
            FT_INT64, BASE_DEC, NULL, 0,
            "Saved group ID", HFILL }
        },
        { &hf_slim_cred_t_euid,
          { "EUID", "tracker.slim_cred_t.euid",
            FT_INT64, BASE_DEC, NULL, 0,
            "Effective user ID", HFILL }
        },
        { &hf_slim_cred_t_egid,
          { "EGID", "tracker.slim_cred_t.egid",
            FT_INT64, BASE_DEC, NULL, 0,
            "Effective group ID", HFILL }
        },
        { &hf_slim_cred_t_fsuid,
          { "FSUID", "tracker.slim_cred_t.fsuid",
            FT_INT64, BASE_DEC, NULL, 0,
            "User ID for VFS operations", HFILL }
        },
        { &hf_slim_cred_t_fsgid,
          { "FSGID", "tracker.slim_cred_t.fsgid",
            FT_INT64, BASE_DEC, NULL, 0,
            "Group ID for VFS operations", HFILL }
        },
        { &hf_slim_cred_t_user_namespace,
          { "User Namespace", "tracker.slim_cred_t.user_namespace",
            FT_INT64, BASE_DEC, NULL, 0,
            NULL, HFILL }
        },
        { &hf_slim_cred_t_secure_bits,
          { "Secure Bits", "tracker.slim_cred_t.secure_bits",
            FT_INT64, BASE_DEC, NULL, 0,
            NULL, HFILL }
        },
        { &hf_slim_cred_t_cap_inheritable,
          { "CapInh", "tracker.slim_cred_t.cap_inheritable",
            FT_INT64, BASE_DEC, NULL, 0,
            "Inheritable capabilities", HFILL }
        },
        { &hf_slim_cred_t_cap_permitted,
          { "CapPrm", "tracker.slim_cred_t.cap_permitted",
            FT_INT64, BASE_DEC, NULL, 0,
            "Permitted capabilities", HFILL }
        },
        { &hf_slim_cred_t_cap_effective,
          { "CapEff", "tracker.slim_cred_t.cap_effective",
            FT_INT64, BASE_DEC, NULL, 0,
            "Effective capabilities", HFILL }
        },
        { &hf_slim_cred_t_cap_bounding,
          { "CapBnd", "tracker.slim_cred_t.cap_bounding",
            FT_INT64, BASE_DEC, NULL, 0,
            "Bounding capabilities", HFILL }
        },
        { &hf_slim_cred_t_cap_ambient,
          { "CapAmb", "tracker.slim_cred_t.cap_ambient",
            FT_INT64, BASE_DEC, NULL, 0,
    ...
    static hf_register_info pktmeta_hf[] = {
        { &hf_pktmeta_src_ip,
          { "Src IP", "tracker.pktmeta.src_ip",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "Source IP", HFILL }
        },
        { &hf_pktmeta_dst_ip,
          { "Dst IP", "tracker.pktmeta.dst_ip",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "Destination IP", HFILL }
        },
        { &hf_pktmeta_src_port,
          { "Src Port", "tracker.pktmeta.src_port",
            FT_UINT32, BASE_DEC, NULL, 0,
            "Source port", HFILL }
        },
        { &hf_pktmeta_dst_port,
          { "Dst Port", "tracker.pktmeta.dst_port",
            FT_UINT32, BASE_DEC, NULL, 0,
            "Destination port", HFILL }
        },
        { &hf_pktmeta_protocol,
          { "Protocol", "tracker.pktmeta.protocol",
            FT_UINT32, BASE_DEC, VALS(ipproto_val), 0,
            "IP protocol", HFILL }
        },
        { &hf_pktmeta_packet_len,
          { "Len", "tracker.pktmeta.packet_len",
            FT_UINT32, BASE_DEC, NULL, 0,
            "Packet length", HFILL }
        },
        { &hf_pktmeta_iface,
          { "Iface", "tracker.pktmeta.iface",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "Interface", HFILL }
        },
    };
    ...
    static hf_register_info dns_hf[] = {
        { &hf_dnsquery_query,
          { "Query", "tracker.dnsquery.query",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "DNS query", HFILL }
        },
        { &hf_dnsquery_type,
          { "Type", "tracker.dnsquery.type",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "DNS query type", HFILL }
        },
        { &hf_dnsquery_class,
          { "Class", "tracker.dnsquery.class",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "DNS query class", HFILL }
        },
        { &hf_dnsanswer_type,
          { "Type", "tracker.dnsanswer.type",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "DNS answer type", HFILL }
        },
        { &hf_dnsanswer_ttl,
          { "TTL", "tracker.dnsanswer.ttl",
            FT_UINT32, BASE_DEC, NULL, 0,
            "DNS answer TTL", HFILL }
        },
        { &hf_dnsanswer_answer,
          { "Answer", "tracker.dnsanswer.answer",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "DNS answer", HFILL }
        },
    };
    ...
    static hf_register_info proto_http_request_hf[] = {
        { &hf_proto_http_request_method,
          { "Method", "tracker.proto_http_request.method",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "HTTP method", HFILL }
        },
        { &hf_proto_http_request_protocol,
          { "Protocol", "tracker.proto_http_request.protocol",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "HTTP protocol version", HFILL }
        },
        { &hf_proto_http_request_host,
          { "Host", "tracker.proto_http_request.host",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "HTTP host", HFILL }
        },
        { &hf_proto_http_request_uri_path,
          { "URI Path", "tracker.proto_http_request.uri_path",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "HTTP URI path", HFILL }
        },
        { &hf_proto_http_request_header,
          { "Header", "tracker.proto_http_request.header",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "HTTP header", HFILL }
        },
        { &hf_proto_http_request_content_length,
          { "Content Length", "tracker.proto_http_request.content_length",
            FT_INT64, BASE_DEC, NULL, 0,
    ...
    static hf_register_info packet_metadata_hf[] = {
        { &hf_packet_metadata_direction,
          { "Direction", "tracker.packet_metadata.direction",
    ...
    static hf_register_info proto_http_hf[] = {
        { &hf_proto_http_direction,
          { "Direction", "tracker.proto_http.direction",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "HTTP direction", HFILL }
        },
        { &hf_proto_http_method,
          { "Method", "tracker.proto_http.method",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "HTTP method", HFILL }
        },
        { &hf_proto_http_protocol,
          { "Protocol", "tracker.proto_http.protocol",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "HTTP protocol version", HFILL }
        },
        { &hf_proto_http_host,
          { "Host", "tracker.proto_http.host",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "HTTP host", HFILL }
        },
        { &hf_proto_http_uri_path,
          { "URI Path", "tracker.proto_http.uri_path",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "HTTP URI path", HFILL }
        },
        { &hf_proto_http_status,
          { "Status", "tracker.proto_http.status",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "HTTP status", HFILL }
        },
        { &hf_proto_http_status_code,
          { "Status Code", "tracker.proto_http.status_code",
            FT_INT32, BASE_DEC, NULL, 0,
            "HTTP status code", HFILL }
        },
        { &hf_proto_http_header,
          { "Header", "tracker.proto_http.header",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "HTTP header", HFILL }
        },
        { &hf_proto_http_content_length,
          { "Content Length", "tracker.proto_http.content_length",
            FT_INT64, BASE_DEC, NULL, 0,
            "HTTP content length", HFILL }
        }
    };
    ...
    static hf_register_info hooked_symbols_hf[] = {
        { &hf_hooked_symbol_entry,
          { "Entry", "tracker.hooked_symbols.entry",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "Hooked symbol entry", HFILL }
        },
        { &hf_hooked_symbol_name,
          { "Name", "tracker.hooked_symbols.name",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "Hooked symbol name", HFILL }
        },
        { &hf_hooked_symbol_module_owner,
          { "Module Owner", "tracker.hooked_symbols.module_owner",
            FT_STRINGZ, BASE_NONE, NULL, 0,
            "Hooked symbol module owner", HFILL }
        }
    };

    module_t *tracker_module;
    ...
    proto_tracker = proto_register_protocol("Tracker", "TRACKER", "tracker");
    proto_register_field_array(proto_tracker, hf, array_length(hf));
    proto_register_field_array(proto_tracker, filter_hf, array_length(filter_hf));
    proto_register_field_array(proto_tracker, network_hf, array_length(network_hf));
    proto_register_field_array(proto_tracker, sockaddr_hf, array_length(sockaddr_hf));
    proto_register_field_array(proto_tracker, slim_cred_t_hf, array_length(slim_cred_t_hf));
    proto_register_field_array(proto_tracker, pktmeta_hf, array_length(pktmeta_hf));
    proto_register_field_array(proto_tracker, dns_hf, array_length(dns_hf));
    proto_register_field_array(proto_tracker, proto_http_request_hf, array_length(proto_http_request_hf));
    proto_register_field_array(proto_tracker, packet_metadata_hf, array_length(packet_metadata_hf));
    proto_register_field_array(proto_tracker, proto_http_hf, array_length(proto_http_hf));
    proto_register_field_array(proto_tracker, hooked_symbols_hf, array_length(hooked_symbols_hf));
    proto_register_subtree_array(ett, array_length(ett));

    tracker_module = prefs_register_protocol(proto_tracker, NULL);

    prefs_register_enum_preference(tracker_module, "pid_format", "PID column format",
        "Whether to show only PID in container, only PID on host, or both",
        &preferences_pid_format, pid_format_vals, FALSE);

    prefs_register_enum_preference(tracker_module, "container_identifier", "Container identifier in column",
    ...
    prefs_register_bool_preference(tracker_module, "container_image", "Show container image",
        "Whether to show the container image in the container column", &preferences_show_container_image);
    
    tracker_tap = register_tap("tracker");
    ...
    event_name_dissector_table = register_dissector_table("tracker.eventName",
        "Tracker event name", proto_tracker, FT_STRINGZ, FALSE);
    
    // initialize process tree
    process_tree_init();
    
    // initialize event enrichment
    register_tracker_enrichments(proto_tracker);

    // find IP dissector for packet dissection
    DISSECTOR_ASSERT((ip_dissector = find_dissector("ip")) != NULL);
}

void proto_reg_handoff_tracker(void)
{
    static dissector_handle_t tracker_json_handle;

    tracker_json_handle = create_dissector_handle(dissect_tracker_json, proto_tracker);
    
    // register to encapsulation dissector table (we use a user-reserved encapsulation)
    dissector_add_uint("wtap_encap", WTAP_ENCAP_USER0, tracker_json_handle);

    register_tracker_statistics();
}